...
Traces store   Tempo monolithic -> S3            tempo-distributed (overkill at this scale)
...
2. **Pod Identity is the seam between doc 01 and doc 03.** The observability workloads that touch
   AWS (Loki→S3, Tempo→S3, External Secrets→Secrets Manager) get their credentials via Pod Identity
...
4. **The OTel Collector is the observability spine** (doc 03): one pipeline ingests OTLP from apps
   ...
   `otlphttp`), traces→Tempo (S3, monolithic). This replaces the old Promtail+scrape-only pattern.
...
6. **Cost + disposability is the through-line** (doc 01 §7): $73 control plane + $32 single NAT +
   spot nodes = ~$110-140/mo floor; `make down` → ~$0 with S3-backed Loki logs + Tempo traces and
   Terraform state surviving. Watch the **$438/mo extended-support trap** — keep the K8s version current.
...
4. ArgoCD sync waves -> ALB controller & Karpenter NodePools (w0) -> ESO (w1) ->
                        kube-prometheus-stack, Loki, Tempo, OTel Operator (w2) ->
